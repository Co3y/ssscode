...
def main():
    ...
    # 初始时从10个执行体中随机选择4个作为服务体
    print("初始时, 从10个异构体中随机选取4个组成服务体S")
    ...
    # 打印服务体
    _E = [data[s[0] - 1], data[s[1] - 1], data[s[2] - 1], data[s[3] - 1]]
    E = np.array(_E)
    # print(E)
    ...
    for var in range(1, 101):
        ...
        # 存放被选中的执行体的序号
        ids = [int(E[0][0]), int(E[1][0]), int(E[2][0]), int(E[3][0])]
        # print(ids)
        ...
        # 存储平均置信度
        wb = openpyxl.load_workbook('n4_confidence.xlsx')
        ws = wb['Sheet']

        ws.cell(row=var + 1, column=1).value = var
        ws.cell(row=var + 1, column=2).value = H

        wb.save('n4_confidence.xlsx')
        ...
        # 存储异构度
        wb = openpyxl.load_workbook('n4_D.xlsx')
        ws = wb['Sheet']
        ...
        if flag == 0:
            ...
            wb = openpyxl.load_workbook('C:/Users/user/Desktop/n4.xlsx')
        ws = wb['异构度生成']
        ...
        wb.save('C:/Users/user/Desktop/n4.xlsx')

        _data_new = pd.read_excel('C:/Users/user/Desktop/n4.xlsx',
        ...
        if len(error) != 0:
            ...
            for i in range(0, 210):
                H.append((data_new[res[i][0]][8] + data_new[res[i][1]][8] + data_new[res[i][2]][8] + data_new[res[i][3]][8]) / n)
                D.append((data_new[res[i][0]][4] + data_new[res[i][1]][4] + data_new[res[i][2]][4] + data_new[res[i][3]][4]) / n)
                C_run.append((data_new[res[i][0]][5] + data_new[res[i][1]][5] + data_new[res[i][2]][5] + data_new[res[i][3]][5]))
            ...
            H_now = (data_new[ids[0] - 1][8] + data_new[ids[1] - 1][8] + data_new[ids[2] - 1][8] + data_new[ids[3] - 1][8]) / n
            D_now = (data_new[ids[0] - 1][4] + data_new[ids[1] - 1][4] + data_new[ids[2] - 1][4] + data_new[ids[3] - 1][4]) / n
            C_run_now = (data_new[ids[0] - 1][5] + data_new[ids[1] - 1][5] + data_new[ids[2] - 1][5] + data_new[ids[3] - 1][5])
            ...
            ret = [i for i in combinations(words, 4)]
            a = {}
            for i in range(0, 210):
                a[i] = ret[i]
            ...
            if len(B) != 0:
                ...
                # 存储系统效益
                wb = openpyxl.load_workbook('n4_Benefit.xlsx')
                ws = wb['Sheet']

                ws.cell(row=var + 1, column=1).value = var
                ws.cell(row=var + 1, column=2).value = list(B.values())[0]

                wb.save('n4_Benefit.xlsx')

                print(Plan_A)
                _E = [data_new[Plan_A[0][0] - 1], data_new[Plan_A[0][1] - 1], data_new[Plan_A[0][2] - 1], data_new[Plan_A[0][3] - 1]]
                E = np.array(_E)
            else:
        ...
        if var % 10 == 0:
            ...
            if len(B) != 0:
                ...
                print(Plan_B)
                _E = [data_new[Plan_B[0][0] - 1], data_new[Plan_B[0][1] - 1], data_new[Plan_B[0][2] - 1], data_new[Plan_B[0][3] - 1]]
                E = np.array(_E)
            else:
